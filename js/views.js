...
function renderDashboard(state, hub) {
   ...
   * lifetime habit.
   *
   *   FLASHCARDS  (21-day ramp): 3 → 15   maintenance, low-impact
   *   LESSONS     (21-day ramp): 1 → 8    knowledge depth, mid
   *   APPS        (14-day ramp): 2 → 20   the actual job-getting bottleneck
   *
   * Re-weighted for job-getting impact, not gamification balance. Apps
   * are the entry condition for offers; mocks (logged separately) are
   * the highest-leverage prep activity per minute. Flashcards and
   * lessons matter but are NOT what gets you hired — they're the floor,
   * not the ceiling. Targets reflect that priority:
   *
   * Flashcards demoted to 15 elite. With per-card XP capped at 2,
   * elite-day flashcard XP = ~30. Compare to 20 apps × 15 = 300 XP
   * (or 600 with morning 2×). The XP economy now says "log apps."
   *
   * Day-0 combined: ~12 min total (3 fc + 1 lesson + 2 apps).
   * Day-21+ elite: ~3-4 hrs/day, dominated by application work.
   *
   * Calendar-day progression: time off doesn't pause the bar. */
  const RAMP_DAYS = { flashcard: 21, lesson: 21, app: 14 };
  const TARGET_START = { flashcard: 3, lesson: 1, app: 2 };
  const TARGET_ELITE = { flashcard: 15, lesson: 8, app: 20 };
  ...
  const buildEffortSVG = (windowDays) => {
    ...
    // Normalize each metric against its OWN day-specific target. After
    // this, 100% means "hit today's goal" for any of the three metrics,
    // and all three goal lines collapse onto one shared horizontal at
    // y=sy(1). Cleaner read than the old elite-normalized version,
    // where the three dashed goal curves climbed at different rates and
    // a 60% reading was ambiguous (which target?).
    const norm = (val, target) => (target > 0 ? val / target : 0);
    const points = (key) =>
      days.map((d, i) => [sx(i), sy(norm(d[key], d[key + '_target']))]);
    const linePath = (key) => smoothPath(points(key));
    const areaPath = (key) => {
      const p = points(key);
      if (!p.length) return '';
      const base = H - PAD_B;
    ...
    const todayDot = (key, color) => {
      const p = days[days.length - 1];
      const x = sx(days.length - 1), y = sy(norm(p[key], p[key + '_target']));
    ...
    // Single shared goal line at y=100% — every metric's "target hit"
    ...
    return `
      <svg viewBox="0 0 ${W} ${H}" width="100%" height="${H}" preserveAspectRatio="none" style="display:block">
        ...
        <!-- Single shared goal line at 100% of today's per-metric target.
        ...
        <path d="${areaPath('flashcard')}" fill="url(#grad-flashcard)"/>
        ...
        <path d="${linePath('lesson')}"
              fill="none" stroke="${COLORS.lesson}"    stroke-width="${strokeW}"
              stroke-linecap="round" stroke-linejoin="round" filter="url(#effort-glow)"/>
        <path d="${linePath('flashcard')}"
              fill="none" stroke="${COLORS.flashcard}" stroke-width="${strokeW}"
              stroke-linecap="round" stroke-linejoin="round" filter="url(#effort-glow)"/>
        ${todayDot('app',       COLORS.app)}
        ${todayDot('lesson',    COLORS.lesson)}
        ${todayDot('flashcard', COLORS.flashcard)}
      </svg>`;
  };
...
const COOLNESS = {
  ...
  // ── 2026-09-16 auto-scored ──
  // The 88 companies that arrived with Los Angeles, London, Paris, Madrid
  // and Barcelona. Same axis as ever: would it land at a downtown dinner.
  'discord':8,
  'epic-games':8,
  'riot-games':8,
  'canva':7,
  ...
  'owkin':6,
  'genesis-molecular':6,
  'caribou-biosciences':5,
  'flatironhealth':6,
  'ultralytics':6,
  'vast-data':7,      // Vast, space stations — not VAST Data
  'camunda':5,
  'sonarsource':5,
  'percona':4,
  'commercetools':4,
  'zefr':5,
  'the-knot-worldwide':5,
  'rocket-lab':7,
  'relativity-space':7,
  'varda-space':7,
  'new-relic':4,
  ...
  'firstup':3,
  'ecovadis':4,
  'applied-systems':6, // Applied Intuition — not Applied Systems
  'thunes':4,
  'tripadvisor':5,
  'criteo':3,
  'zeta-global':3,
...
const QUANT_GATED = new Set([
  // 2026-09-16 auto — XTX Markets, onto the board with London.
  'xtx-markets',
